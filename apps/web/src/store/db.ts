...
async function instantiateScaffold(projectId: string, specs: ScaffoldNode[], parentId: string | null) {
  ...
  for (const spec of specs) {
    ...
    await db.nodes.add({
      ...
      content: "", ink: "", order: order++, wordCount: 0, originId: "", createdAt: t, updatedAt: t,
    });
...
export const nodes = {
  ...
  async create(input: { projectId: string; parentId: string | null; type: NodeType; title: string; originId?: string }): Promise<StoryNode> {
    ...
    const row: NodeRow = {
      ...
      synopsis: "", content: "", ink: "", order, wordCount: 0, originId: input.originId ?? "", createdAt: t, updatedAt: t,
    };
    await db.nodes.add(row);
    return toNode(row);
  },
  async update(nid: string, patch: Partial<Pick<StoryNode, "title" | "synopsis" | "content" | "ink" | "order" | "parentId">>): Promise<StoryNode | undefined> {
